<template>
    <div class="pagination-list text-typography_primary">
        ...
        <nuxt-link v-show="currentPage > 1" class="pagination-item pagination-icon" :to="prevLink">
            <IconsChevronDown class="w-6 h-6 transform rotate-90" width="24" height="24" />
        </nuxt-link>
        <!-- First Page -->
        ...
        <template v-for="page in pageRange" :key="page">
            <nuxt-link v-show="page !== 1 && page !== totalPages"
                :class="['pagination-item', currentPage === page ? 'active' : '']" :to="getPageUrl(page)">{{ page
                }}</nuxt-link>
        </template>
        <!-- ... -->
        ...
        <!-- Last Page -->
        <nuxt-link v-show="totalPages > 1" :class="['pagination-item', currentPage === totalPages ? 'active' : '']"
            :to="getPageUrl(totalPages)">{{ totalPages }}</nuxt-link>
        <!-- Chevron -->
        <nuxt-link v-show="currentPage < totalPages" class="pagination-item pagination-icon"
